...
这种情况下在集成时需要客户在用户进行登录 / 注册 / 完善个人资料等情况下调用 `alias` 接口获得注册 ID，使得匿名 ID 实名化，然后 SDK 就会将数据的 `$is_login` 属性变成 `true`，表示这是一个注册 ID，后续的行为中上报的`xwho`也会有注册ID；当用户在未登录前或者登录后退出的情况下，行为中`xwho` 中上传的是匿名 ID。
